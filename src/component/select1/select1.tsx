import React, { Dispatch, forwardRef, ReactNode, SetStateAction, useDeferredValue, useEffect, useImperativeHandle, useMemo, useRef, useState } from 'react'
...
interface Select1Props {
    ...
    style?: Array<ViewStyle | TextStyle | WSelect1Variant> | ViewStyle | TextStyle | WSelect1Variant
    prefix?: ReactNode,
    suffix?: ReactNode,
    simpleStyle?: boolean
...
export const WSelect1 = forwardRef<Select1Ref, Select1Props>(({ style = initStyle, helperTextColor = "#E14337", ...props }, ref) => {
    ...
    const onOpenOptions = () => {
        showBottomSheet({
            ...
            enableDismiss: true,
            style: { height: 240 },
            children: props.customOptionsList ?? <OptionDropList
        ...
        >
            ...
            {typeof valueItem?.name === "object" ?
                ...
                <Text numberOfLines={1}
                    ...
                    }}>{valueItem?.name ?? props.placeholder}</Text>}
...
const OptionDropList = (props: { selected?: string | number, onSelect: (e: OptionsItem) => void, getOptions: (params: { length: number, search?: string, parentId?: string | number }) => Promise<{ data: Array<OptionsItem>, totalCount: number }>, }) => {
    ...
    return <>
        {options.totalCount === 0 && !initTotal.current ? <View style={{ alignItems: "center", marginVertical: 8 }}>
            <Winicon src='color/files/archive-file' size={28} />
            <Text style={[textStyles?.['heading-7'], { margin: 8 }]}>{t("noResultFound")}</Text>
        </View> :
            <>
                {initTotal.current && initTotal.current > 10 && <WTextField
                    style={[textStyles?.['body-3'] ?? {}, WTextFieldVariant.size32, { marginVertical: 8, marginHorizontal: 16 }]}
...
function OptionsItemTile({ item, children, selected, onPress, getOptions }: OptionTileProps) {
    ...
    useEffect(() => {
        if (children?.length && children.some(e => e.id === selected)) setIsOpen(true)
    }, [children?.length, selected])

    return <>
        {<TouchableOpacity style={[styles.selectTile, { backgroundColor: selected === item.id ? colors?.['neutral-background-color-selected'] : item.disabled ? colors?.['neutral-background-color-disable'] : undefined }]}
            onPress={() => {
            ...
            {typeof item?.name === "string" ? <>
                ...
                {item.prefix}
                <Text style={[textStyles?.['label-4'], { flex: 1, color: item.disabled ? colors?.['neutral-text-color-disabled'] : colors?.['neutral-text-color-label'] }]}>{item.name}</Text>
            </> : item.name}
        </TouchableOpacity>}
        {isOpen && <>
            {options.data.map((child, i) => {
                return <TouchableOpacity key={child.id + "-" + i}
                    style={[styles.selectTile, { paddingLeft: 32, backgroundColor: selected === child.id ? colors?.['neutral-background-color-selected'] : child.disabled ? colors?.['neutral-background-color-disable'] : undefined }]}
                    onPress={() => onPress(child)}
                >
                    {typeof child?.name === "string" ? <>
                        {child.prefix}
                        <Text style={[textStyles?.['label-4'], { flex: 1, color: child.disabled ? colors?.['neutral-text-color-disabled'] : colors?.['neutral-text-color-label'] }]}>{child.name}</Text>
                    </> : child.name}
                </TouchableOpacity>
            })}
            {options.data.length < options.totalCount &&
                <TouchableOpacity style={{ paddingVertical: 4, paddingHorizontal: 8, marginLeft: 4, marginBottom: 2 }}
                    onPress={() => {
