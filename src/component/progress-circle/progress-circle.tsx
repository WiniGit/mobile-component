...
export const WProgressCircle = ({ strokeWidth = 4, percent = 0, ...props }: ProgressCircleProps) => {
    ...
    return <SvgXml
        xml={`<svg width="100%" height="100%" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M 30,30 m 0,-${radius} a ${radius},${radius} 0 1 1 0,${2 * radius} a ${radius},${radius} 0 1 1 0,-${2 * radius}" stroke="${props.percentColor ?? colors?.["primary-color-main"] ?? "#287CF0"}" stroke-width="${strokeWidth}" stroke-dasharray="${diameter}px ${diameter}px" stroke-dashoffset="${strokeOffset}px" stroke-linecap="round" />
