...
class CustomSnackbar extends react_1.default.Component {
    state = {};
    onShow = (newState) => {
    ...
    getBg = () => {
        switch (this.state.status) {
            case component_status_1.ComponentStatus.WARNING:
                return color_1.lightThemeColor['warning-color-main'];
            case component_status_1.ComponentStatus.ERROR:
                return color_1.lightThemeColor['error-color-main'];
            case component_status_1.ComponentStatus.SUCCSESS:
                return color_1.lightThemeColor['success-color-main'];
            default: // ComponentStatus.INFOR
                return color_1.lightThemeColor['infor-color-main'];
        }
    };
    getIcon = () => {
        switch (this.state.status) {
            case component_status_1.ComponentStatus.INFOR:
                return ((0, jsx_runtime_1.jsx)(react_native_svg_1.SvgXml, { xml: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    render() {
        return ((0, jsx_runtime_1.jsx)(react_native_paper_1.Snackbar, { wrapperStyle: { zIndex: 1 }, style: {
                zIndex: 100,
                backgroundColor: this.getBg(),
                bottom: this.state.bottom ?? 16,
            }, visible: this.state.visible === true, duration: 2000, onDismiss: () => {
                this.setState({ ...this.state, visible: false });
            }, children: this.state.children ?? ((0, jsx_runtime_1.jsxs)(react_native_1.View, { style: {
                    flexDirection: 'row',
                    columnGap: 12,
                    alignItems: 'flex-start',
                }, children: [this.getIcon(), (0, jsx_runtime_1.jsxs)(react_native_1.View, { style: { columnGap: 8, flex: 1 }, children: [!!this.state.label?.length && (0, jsx_runtime_1.jsx)(react_native_paper_1.Text, { numberOfLines: 1, style: [typography_1.typography['heading-7'], { color: '#ffffff' }], children: this.state.label ?? '' }), !!this.state.message?.length && (0, jsx_runtime_1.jsx)(react_native_paper_1.Text, { numberOfLines: 2, style: [typography_1.typography['body-3'], { color: '#ffffff' }], children: this.state.message ?? '' })] }), !!this.state.actionTitle?.length && (0, jsx_runtime_1.jsx)(react_native_1.TouchableOpacity, { onPress: this.state.action, children: (0, jsx_runtime_1.jsx)(react_native_paper_1.Text, { style: [typography_1.typography['label-4'], { color: '#ffffff' }], children: this.state.actionTitle }) })] })) }));
    }
}
const snackbarRef = (0, react_1.createRef)();
function FSnackbar() {
    return (0, jsx_runtime_1.jsx)(CustomSnackbar, { ref: snackbarRef });
}
const showSnackbar = ({ message, label, status, children, action, actionTitle, bottom, }) => {
